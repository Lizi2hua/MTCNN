...
class Detector:
    ...
    def __r_detect(self, image, pnet_boxes):
        ...
        # img=img_crop.permute(0,2,3,1)
        # for i in range(img.shape[0]):
        #     plt.imshow(img[i])
        #     plt.show()
        # exit()
        # print("rnet img crop:",img_crop)
        if self.isCuda:
    ...
    # 由于R,O网络输入必须是矩形，倘若简单的使用resize，回破坏原图的特征。可以将pnet输出的框扩张为正方形
    def convert_to_square(self, bbox):
        ...
        square_bbox[:, 1] = (bbox[:, 1] + w * 0.5) - max_side * 0.5  #()内找中心
